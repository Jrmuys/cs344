...
def load_data(h = False):
    temp = np.load('character_font.npz')
    return temp['images'], temp['labels']

# Load alphabet font data
...
for step in range(iterations):
    ...
    if step % 100 == 0:
        ...
        img.save(os.path.join(save_dir, 'generated_letter' + str(step) + '.png'))

        # Save one real image, for comparison
        img = image.array_to_img(real_images[0] * 255., scale=False)
        img.save(os.path.join(save_dir, 'real_letter' + str(step) + '.png'))
